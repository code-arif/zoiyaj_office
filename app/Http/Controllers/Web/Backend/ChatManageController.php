...
class ChatManageController extends Controller
{
    ...
    public function list(): JsonResponse
    {
        ...
        $users = User::select('id', 'first_name', 'email', 'avatar', 'last_activity_at')
    ...
    public function search(Request $request)
    {
        ...
        $users   = User::select('id', 'first_name', 'email', 'avatar', 'last_activity_at')
            ->where('id', '!=', $user_id)
            ->where('first_name', 'LIKE', "%{$keyword}%")->orWhere('email', 'LIKE', "%{$keyword}%")
            ->get();
    ...
    public function conversation($receiver_id): JsonResponse
    {
        ...
        $data = [
            'receiver' => User::select('id', 'first_name', 'email', 'avatar', 'last_activity_at')->where('id', $receiver_id)->first(),
            'sender'   => User::select('id', 'first_name', 'email', 'avatar', 'last_activity_at')->where('id', $sender_id)->first(),
            'room'     => $room,
            'chat'     => $chat,
        ];
    ...
    public function room($receiver_id)
    {
        ...
        $room = Room::with(['first_user:id,first_name,email,avatar,last_activity_at', 'second_user:id,first_name,email,avatar,last_activity_at'])
            ->where(function ($query) use ($receiver_id, $sender_id) {
                $query->where('first_user_id', $receiver_id)->where('second_user_id', $sender_id);
            })->orWhere(function ($query) use ($receiver_id, $sender_id) {
            $query->where('first_user_id', $sender_id)->where('second_user_id', $receiver_id);
